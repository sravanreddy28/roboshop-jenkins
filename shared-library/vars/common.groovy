...
def codequality() {
    stage('Code Quality') {
        ...
        wrap([$class: "MaskPasswordsBuildWrapper", varPasswordPairs: [[password: sonarpass]]]) {
            sh 'sonar-scanner -Dsonar.host.url=http://172.31.37.39K:9000 -Dsonar.login=${sonaruser} -Dsonar.password=${sonarpass} -Dsonar.projectKey=${component} -Dsonar.qualitygate.wait=true'
        }
    }
}
